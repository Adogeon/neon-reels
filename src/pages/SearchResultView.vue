...
<template>
    ...
    <section v-else id="result-area">
        <div v-if="selectMovie"  class="select-movie">
            ...
            <div><RouterLink :to="`/details/${selectMovie.id}`">Further Details ...</RouterLink></div>
            <button @click.stop="selectMovie=null">Close</button>
        </div>
        <div class="result-grid">
           <MovieSimpleCard v-for="movie in movies" :data="movie" :key="`movie-${movie.id}`" @click="toggleMovieDetails(movie)"/>
        </div>
    </section>
</template>
